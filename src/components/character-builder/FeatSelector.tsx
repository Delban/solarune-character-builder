...
export function FeatSelector() {
  ...
  // Calculer les dons utilisés par slot de manière plus précise
  const usedFeatsBySlot = useMemo(() => {
    const result: { [slotId: string]: string[] } = {};
    
    // Initialiser tous les slots avec des tableaux vides
    featSlots.forEach(slot => {
      result[slot.id] = [];
    });
    
    // Pour chaque niveau, distribuer les dons aux slots appropriés
    character.niveaux.forEach((levelData, levelIndex) => {
      const globalLevel = levelIndex + 1;
      const levelSlots = featSlots.filter(slot => slot.level === globalLevel);
      
      if (levelSlots.length > 0 && levelData.donsChoisis.length > 0) {
        // Distribuer les dons aux slots du niveau
        // Pour l'instant, on met tous les dons dans le premier slot disponible
        // Cela pourrait être amélioré pour une distribution plus intelligente
        levelData.donsChoisis.forEach((featId, featIndex) => {
          const slotIndex = featIndex % levelSlots.length;
          const targetSlot = levelSlots[slotIndex];
          if (targetSlot) {
            result[targetSlot.id].push(featId);
          }
        });
      }
    });
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {featSlots.map((slot) => {
          ...
          const availableFeats = getAvailableFeatsForSlot(slot)
            .filter(don => {
              // Filtres globaux
